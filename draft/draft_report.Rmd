...
Despite the fact that typeracer maintains this list of races for each player, there is no publicly available list of players. To find metadata about users (specifically their user ids), we are utilizing a third-party website, **[typeracerdata](https://typeracerdata.com/leaders)**, that aggregates and logs racer ids and some associated statistics for viewing. Players can manually enter themselves into **typeracerdata** and then can query themselves for updates. 
...
These assumptions will be further explored in later parts of this report, specifically in data exploration.
...
Below is a list of the racer, along with total races typed and their improvement proportional to their starting estimated wpm:
...
Below are some general statistics about our dataset, and brief description on what they mean.

The below summary provides some brief statistics on the entire dataset. For additional explanation, the `average_racers` column indicates the average number of racers on a track.
```{r echo=FALSE}
summary2 = data %>% 
  summarize(
...
The below summary provides some brief statistics on improvement for the 100 typists sampled.
```{r echo=FALSE}
summary = improvement %>% 
  summarize(
    total = n(),
...
Something relatively unique about typeracer form traditional typing software tools is the ability to race with other typists. We can use our dataset to analyze whether there are differences in the mean wpm and accuracy for typists based on the numbers of other typists present on the racetrack, evaluating whether or not there is a competitive advantage to racing with others. While this isn't directly related to typist improvement over races, it still could provide hints on how typeracer provides this improvement.
...
Based on the extremely low counts of 6 - 10 racers on a track, it's possible that those races were on older versions of the typeracer website. It seems like the most frequent races occur with 5 racers. If we eliminate 6 - 10 due to the very low counts of races logged to make the average, it seems like 2 typist races - 1v1s - seem to have the highest average WPM.
...
```
The test statistic is `r round(test$statistic, 3)`. 
The p-value is `r test$p.value`.

The confidence interval values span from `r round(test$conf.int[1], 3)` upwards.

## Discussion

### Interpreting Results

There is overwhelming evidence that the mean improvement, by proportion, for a typist relative to their starting estimated wpm over races is greater than 0 (p is `r test$p.value`, one-sided t-test, df=99).
...
We are 95% confident that the true mean improvement of dedicated typeracer users, relative to their starting wpm as a proportion, is greater than `r round(test$conf.int[1], 3)`.
...
- Typeracer as a software has evolved over time from 2010 to 2021. While data spans back to 2010, we have not considerably evaluated text variety and text difficulty differences between times, which may skew our data.